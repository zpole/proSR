...
class ToVggInput(nn.Module):
    ...
    def __init__(self, orig_mean, orig_std):
        ...
        self.orig_std = nn.Parameter(
            torch.Tensor(orig_std).view(1, 3, 1, 1), requires_grad=False)
    ...
    def forward(self, x):
        return (x * self.orig_std + self.orig_mean - self.mean) / self.std
...
class Vgg16(nn.Module):
    def __init__(self,
        ...
        layers = OrderedDict()
        layers['data_conversion'] = ToVggInput(orig_mean, orig_std)
        for i in range(upto):
            features = []
